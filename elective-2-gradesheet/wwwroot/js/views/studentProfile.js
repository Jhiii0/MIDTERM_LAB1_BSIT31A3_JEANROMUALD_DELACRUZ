...
document.addEventListener('DOMContentLoaded', function () {
            ...
            try {
                ...
                if (result.success && result.addedCount > 0) {
                    setTimeout(() => {
                        ...
                        window.location.href = `${baseUrl}?period=${gradingPeriod}`;
    ...
    const toastTypeElement = document.querySelector('[data-toast-type]');
    
    if (toastMessageElement && toastTypeElement) {
        const toastMessage = toastMessageElement.getAttribute('data-toast-message');
        const toastType = toastTypeElement.getAttribute('data-toast-type');
